...
(function (window) {
    ...
    queryly.ask = {
        renderAsk: function (results, target,template) {
            ...
            try {
                for (var i = 0; i < results.gpts.length; i++) {
                    var gpt = results.gpts[i];
                    if (gpt.answer.length == 0) { continue; }
                    var docids = gpt.docs.map(obj => obj._id);
                    gpthtml = gpthtml + '<div class="queryly_item_container queryly_item_container_ask"><div class="queryly_ask_button" style="float:right;margin-left:10px;display:none;"><img src="https://www.queryly.com/images/chat.png" style="width: 24px;vertical-align:top;margin-right:6px;display:inline">Ask me</div><div class="queryly_item_title queryly_item_title_ask" style="color:green;">' + gpt["title"] + '</div><div class="queryly_item_description queryly_item_description_ask" style="color:#333333;">';
                    for (var j = 0; j < gpt.answer.length; j++) {
                        gpthtml = gpthtml + gpt.answer[j]["text"].substring(0, gpt.answer[j]["text"].length - 1);
                        for (var k = 0; k < gpt.answer[j].references.length; k++) {
                            var docid = gpt.answer[j].references[k];
                            var num = docids.indexOf(docid);
                            var skip_ref = (gpt.answer.length > j + 1 && gpt.answer[j + 1].references.length > 0 && docids.indexOf(gpt.answer[j + 1].references[0]) == num) ? true : false;
                            if (num >= 0 && !skip_ref) {
                                gpthtml = gpthtml + '<span data-id="' + docid + '" class="numberCircle"><a target="_blank" style="color:white;" href="' +  gpt.docs[num]["link"] + '">' + (num + 1) + "</a></span>";
                            }
                        }
                        gpthtml = gpthtml + ". ";
                    }

                    gpthtml = gpthtml + "<div class='queryly_reference_container' style='font-size: 11px;line-height:16px;margin-top:10px;'>";
                    var count = 0;
                    for (var j = 0; j < gpt.docs.length; j++) {
                        gpthtml = gpthtml + "<div><a style='color: #777;text-decoration:none;' target='_blank' href='" + gpt.docs[j]["link"] + "'>" + (count + 1) + ". " + gpt.docs[j]["title"] + "</a></div>";
                        count = count + 1;
                    }
                    gpthtml = gpthtml + "</div></div>";

                }
                if (gpthtml != "") {
                    elem = document.createElement("div");
                    elem.innerHTML = gpthtml;
                    target.appendChild(elem.children[0]);
                }
            }
            catch (e) { return null;}            
