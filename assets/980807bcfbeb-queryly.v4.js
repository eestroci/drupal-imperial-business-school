...
(function (window) {
    ...
    queryly.ask = {
        ...
        summarize: async function (apikey, keyword) {
            elem = document.createElement("div");
            elem.innerHTML = '<img class="loadingicon" src="https://www.queryly.com/images/loadingdots.svg" style="width: 40px;display: inline;vertical-align: top;">';
            if (document.querySelector('#queryly_summarize') != null) {
                document.querySelector('#queryly_summarize').insertAdjacentElement("afterend", elem.children[0]);
                document.querySelector('#queryly_summarize').remove();
            }

            var apiUrl = 'https://www.queryly.com/ask/json.aspx?call=searchsummary&queryly_key=' + apikey + '&keyword=' + encodeURIComponent(keyword);
            if (typeof queryly.ask.related_questions != 'undefined') {
                apiUrl = apiUrl + "&related_questions=" + queryly.ask.related_questions;
            }
            const data = {};
            // Parse the JSON data
